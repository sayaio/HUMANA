...
import { API_URL } from '../src/config'; // ← tambah ini, sesuaikan path
  ...
  const handlePaymentPress = async () => {
    ...
    if (selectedMethod === 'cod') {
      try {
        ...
        if (response.ok) {
          // 🛠️ PERBAIKAN: Langsung panggil Alert tanpa requestAnimationFrame
          Alert.alert(
            'Sukses',
          ...
          // 🚀 SOS / FALLBACK CADANGAN: 
          // Jika dalam 500ms Alert di atas nge-bug atau diblokir sistem Android,
          // aplikasi akan dipaksa langsung back ke halaman utama secara otomatis!
          setTimeout(() => {
        ...
        // Gunakan Alert langsung secara bersih
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.materiSection}>
          <Text style={styles.subjectTitle}>{sessionData?.nama_mapel || 'Kimia'}</Text>
          <Text style={styles.chapterText}>{sessionData?.nama_materi || 'Struktur Atom'}</Text>
          <View style={styles.divider} />
          <Text style={styles.gradeText}>SMA - Kelas 12</Text>
        </View>

        {/* CARD UTAMA */}
        <View style={styles.mainCard}>
          <Text style={styles.inputLabel}>Lokasi Belajar</Text>
          <TextInput style={styles.disabledInput} value="Kampus" editable={false} />
          <View style={styles.locationWrapper}>
            <Text style={styles.pinIcon}>📍</Text>
            <TextInput style={[styles.disabledInput, { flex: 1, borderWidth: 0, paddingLeft: 30 }]} value={sessionData?.lokasi || "37.42200, -122.08400"} editable={false} />
          </View>

          <Text style={styles.inputLabel}>Jadwal Pemesanan</Text>
          <TextInput style={styles.disabledInput} value={sessionData?.tanggal || "25 Mei 2026"} editable={false} />

          <Text style={styles.inputLabel}>Durasi sesi</Text>
          <TextInput style={styles.disabledInput} value={sessionData?.waktu_sesi || "06:30 - 07:30"} editable={false} />
        </View>
      ...
      {/* TOMBOL UTAMA (BERUBAH TEKS & WARNA SEUSAI METODE) */}
...
const styles = StyleSheet.create({
  ...
  customModalOverlay: {
    ...
    backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end', zIndex: 9999, elevation: 999,
  },
  bottomSheetContainer: {
    backgroundColor: '#FFF', borderTopLeftRadius: 30, borderTopRightRadius: 30,
    paddingHorizontal: 24, paddingTop: 15, paddingBottom: 50, zIndex: 10000, elevation: 1000,
  },
  ...
  imageOptionBox: {
    ...
    elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.03, shadowRadius: 3
  },
  selectedOptionBox: {
    borderColor: '#3A7D6B',
    borderWidth: 1.5,
