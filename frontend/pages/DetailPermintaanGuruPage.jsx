...
const DetailPermintaanGuruPage = ({ permintaanData, onBack, onTerima, onTolak }) => {
  // Fallback aman jika data belum lengkap
  ...
  // Buka Google Maps menggunakan koordinat atau alamat
  const handleBukaMap = () => {
    const lokasi = data.lokasi_sesi || data.lokasi || '';
    if (lokasi) {
      // Coba parse koordinat lat,lng
  ...
  const handleTerima = () => {
    Alert.alert(
      'Terima Permintaan',
      `Yakin ingin menerima permintaan dari ${data.nama_murid}?`,
      [
        { text: 'Batal', style: 'cancel' },
        {
          text: 'Terima',
          onPress: () => {
            if (onTerima) onTerima(data.id_pemesanan || data.id);
          },
        },
      ]
    );
  ...
  // Format tanggal dari waktu_mulai
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.actionBar}>
        ...
        <TouchableOpacity style={styles.btnTerima} onPress={handleTerima} activeOpacity={0.8}>
